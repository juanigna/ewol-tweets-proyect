        ...
        reader.onload = async (readerEvent) => {
            if (mimeType.split('/')[1] === 'tiff') {
                ...
                image.onload = async (e) => {
                    const currentImgCanvas = getProcessedImgCanvas(
                        image,
                        readerEvent
                    );
...
export const downScaleCanvas = async (
    ...
    if (scale < 1 && scale > 0) {
        ...
        var tw =
            cv.width === cv.height
                ? imgConfigs.resolution
                : Math.floor(sw * scale); // target image width
        var th =
            cv.width === cv.height
                ? imgConfigs.resolution
                : Math.floor(sh * scale); // target image height
...
export const getBase64Size = (base64string, multiplier) => {
    ...
    if (multiplier) {
        ...
        else if (multiplier === 'MiB' || multiplier === 'mib')
            divider = 1048576;
        else if (multiplier === 'GiB' || multiplier === 'gib')
            divider = 1073741824;
        else if (multiplier === 'KB' || multiplier === 'kb') divider = 1024;
        else if (multiplier === 'MB' || multiplier === 'mb') divider = 1048576;
        else if (multiplier === 'GB' || multiplier === 'gb')
            divider = 1073741824;
        else divider = 1;
    }
